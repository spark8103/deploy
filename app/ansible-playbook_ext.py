...
ansible_dir = os.path.join(Config.BASE_DIR, 'ansible')
ansible_inventory_file = os.path.join(ansible_dir, 'inventory_prod')
...
def ansible_playbook_task(self, playbook, hosts, type='ansible-playbook'):
    runner = Runner(
        playbook=playbook,
        hosts=ansible_inventory_file,
        display=display,
        options={
            'subset': hosts,
            # 'become': True,
            # 'become_method': 'sudo',
            # 'become_user': 'root',
            'private_key_file': Config.ANSIBLE_KEY,
            'tags': '',
            'skip_tags': '',
            # 'verbosity': 0,
        },
...
def get_playbook_file():
    """ get get_playbook file list """
